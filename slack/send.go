...
func SendMessage(message string, attachment slack.Attachment) {
	...
	channelID, timestamp, err := api.PostMessage(c.Slack.ChannelID, slack.MsgOptionText(message, true), slack.MsgOptionAttachments(params.Attachments...))
	if err != nil {
		fmt.Printf("%s\n", err)
		return
	}
...
func SendDirectMessage(message string, attachment slack.Attachment, user string) {
	...
	api.PostMessage(channelID, slack.MsgOptionText(message, true), slack.MsgOptionAttachments(params.Attachments...))
}
